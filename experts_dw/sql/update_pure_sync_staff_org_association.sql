...
WHEN MATCHED
  THEN UPDATE SET
    ...
    ps.modified = SYSDATE
  WHERE
    ORA_HASH(ps.person_id || ps.period_start_date || ps.period_end_date || ps.org_id || ps.employment_type || ps.staff_type || ps.visibility || ps.primary_association || ps.job_description || ps.affiliation_id)
    <>
    ORA_HASH(pss.person_id || pss.period_start_date || pss.period_end_date || pss.org_id || pss.employment_type || pss.staff_type || pss.visibility || pss.primary_association || pss.job_description || pss.affiliation_id)
