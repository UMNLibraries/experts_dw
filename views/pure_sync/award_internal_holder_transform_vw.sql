...
CREATE OR REPLACE FORCE EDITIONABLE VIEW expert.pure_sync_award_internal_holder_transform_vw (
  ...
  SELECT
    award_person.*,
    ROW_NUMBER() OVER (
      -- This partitions by both columns, first award_id, then person_id:
      PARTITION BY award_person.award_id, award_person.person_id
  ...
  FROM (
    ...
    JOIN pure_sync_person_data pure_person
      ON pure_person.emplid = award.emplid
